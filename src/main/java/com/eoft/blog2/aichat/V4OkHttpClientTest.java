...
public class V4OkHttpClientTest {
    ...
    private static final String API_SECRET_KEY = "c8101a375c360d97c8cd0cd86e261320.ohECIIbey17cN2ia";
...
//    //     4.异步查询
        String taskId = testAsyncInvoke2("你好");
//        String  taskId = "676317108788568948494922287647953873";
...
//        request.setTraining_file("file-20240320222135-eeettt");
...
//        createImageRequest.setPrompt("画一个温顺可爱的小狗");
     ...
     * sse调用
     */
    public static void testSseInvoke(String message1) {
        List<ChatMessage> messages = new ArrayList<>();
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), message1);
...
//        这里需要遍历输入啊啊呀
    ...
    public static String testAsyncInvoke2(String messages1) {
        List<ChatMessage> messages = new ArrayList<>();
        ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), messages1);
        messages.add(chatMessage);
        String requestId = String.format(requestIdTemplate, System.currentTimeMillis());
        // 函数调用参数构建部分
        List<ChatTool> chatToolList = new ArrayList<>();
...
//        chatToolList.add(chatTool);
     ...
     * @param taskId
     */
    public static String testQueryResult(String taskId) {
        ...
        try {
            return  mapper.writeValueAsString(queryResultResp);
        } catch (JsonProcessingException e) {
            e.printStackTrace();
        }
        return "我觉得这步不可以执行到这里";
    }
}
